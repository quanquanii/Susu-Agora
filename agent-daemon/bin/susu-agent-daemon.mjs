...
class PromptCachingBetaMessageStream {
  constructor() {
    ...
    _PromptCachingBetaMessageStream_handleError.set(this, (error) => {
      __classPrivateFieldSet6(this, _PromptCachingBetaMessageStream_errored, true, "f");
      if (error instanceof Error && error.name === "AbortError") {
        error = new APIUserAbortError;
      }
      if (error instanceof APIUserAbortError) {
        __classPrivateFieldSet6(this, _PromptCachingBetaMessageStream_aborted, true, "f");
  ...
  _emit(event, ...args) {
    if (__classPrivateFieldGet7(this, _PromptCachingBetaMessageStream_ended, "f"))
      return;
    if (event === "end") {
      __classPrivateFieldSet6(this, _PromptCachingBetaMessageStream_ended, true, "f");
      __classPrivateFieldGet7(this, _PromptCachingBetaMessageStream_resolveEndPromise, "f").call(this);
    }
    const listeners = __classPrivateFieldGet7(this, _PromptCachingBetaMessageStream_listeners, "f")[event];
    if (listeners) {
      __classPrivateFieldGet7(this, _PromptCachingBetaMessageStream_listeners, "f")[event] = listeners.filter((l2) => !l2.once);
      listeners.forEach(({ listener }) => listener(...args));
    }
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet7(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, "f") && !listeners?.length) {
  ...
  [(_PromptCachingBetaMessageStream_currentMessageSnapshot = new WeakMap, _PromptCachingBetaMessageStream_connectedPromise = new WeakMap, _PromptCachingBetaMessageStream_resolveConnectedPromise = new WeakMap, _PromptCachingBetaMessageStream_rejectConnectedPromise = new WeakMap, _PromptCachingBetaMessageStream_endPromise = new WeakMap, _PromptCachingBetaMessageStream_resolveEndPromise = new WeakMap, _PromptCachingBetaMessageStream_rejectEndPromise = new WeakMap, _PromptCachingBetaMessageStream_listeners = new WeakMap, _PromptCachingBetaMessageStream_ended = new WeakMap, _PromptCachingBetaMessageStream_errored = new WeakMap, _PromptCachingBetaMessageStream_aborted = new WeakMap, _PromptCachingBetaMessageStream_catchingPromiseCreated = new WeakMap, _PromptCachingBetaMessageStream_handleError = new WeakMap, _PromptCachingBetaMessageStream_instances = new WeakSet, _PromptCachingBetaMessageStream_getFinalMessage = function _PromptCachingBetaMessageStream_getFinalMessage2() {
    ...
    if (this.ended)
      return;
    __classPrivateFieldSet6(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, undefined, "f");
    ...
    switch (event.type) {
      ...
      case "message_start": {
        __classPrivateFieldSet6(this, _PromptCachingBetaMessageStream_currentMessageSnapshot, messageSnapshot, "f");
        break;
      }
...
var Headers3 = undefined;
var FormData3 = undefined;
var Blob3 = undefined;
var File3 = undefined;
var ReadableStream4 = undefined;
var getMultipartRequestOptions3 = undefined;
var getDefaultAgent2 = undefined;
var fileFromPath4 = undefined;
...
var defaultHttpAgent2 = new import_agentkeepalive2.default({ keepAlive: true, timeout: 300000 });
var defaultHttpsAgent2 = new import_agentkeepalive2.default.HttpsAgent({ keepAlive: true, timeout: 300000 });
...
var TOOL_NOOP = {
  name: "do_nothing",
  description: "Choose this when no response is warranted: not your turn, ambiguous, low confidence, or the event is informational only.",
  parameters: {
    type: "object",
    properties: {
...
var TOOL_PUSH = {
  name: "push_signal",
  description: "Push a fresh signal into the channel. Use for new alpha, asks, or context the channel doesn't have yet. Avoid spamming — `do_nothing` is often the right call.",
  parameters: {
    type: "object",
    properties: {
...
class AnthropicProvider {
  ...
  async decide(ctx, systemPrompt) {
    ...
    const userText = JSON.stringify({
      ...
      instructions: "Choose exactly ONE tool call: do_nothing / react_to_signal / push_signal. Be conservative — prefer do_nothing if uncertain."
    }, null, 2);
    ...
    for (const block of resp.content) {
      if (block.type === "tool_use") {
        return { decision: blockToDecision(block.name, block.input), stats };
      }
    }
    return {
      decision: { kind: "noop", reason: "LLM returned no tool call" },
      stats
    };
  }
}
...
function blockToDecision(toolName, input) {
  switch (toolName) {
    case "do_nothing":
...
function dim(s2) {
  return isTTY() ? `${DIM}${s2}${RESET}` : s2;
}
function color(s2, c2) {
  return isTTY() ? `${c2}${s2}${RESET}` : s2;
}
function shortId(id) {
  return id.slice(0, 8);
}
function fmtTime() {
  const d2 = new Date;
  return d2.toISOString().replace("T", " ").replace(/\.\d+Z$/, "Z");
}
...
class DecisionLog {
  ...
  printToTerminal(e2) {
    const t2 = dim(fmtTime());
    const triggerPreview = previewEvent(e2.ctx.triggering_event);
    process.stdout.write(`${t2}  ${color(e2.ctx.channel_label, CYAN)}  ${triggerPreview}
`);
    process.stdout.write(`  ${dim("⌥ context:")} ${e2.ctx.recent_events.length} recent events
`);
    process.stdout.write(`  ${dim("⌥ LLM:")} ${e2.stats.provider}/${e2.stats.model}  ` + `${e2.stats.input_tokens}↑ ${e2.stats.output_tokens}↓ tok  ` + `${(e2.stats.latency_ms / 1000).toFixed(2)}s
`);
    const decisionLine = formatDecision(e2.decision);
    process.stdout.write(`  ${dim("⌥ decision:")} ${decisionLine}
`);
    if (e2.error) {
      process.stdout.write(`  ${color("⌥ ERROR:", RED)} ${e2.error}
`);
    } else if (e2.result) {
      process.stdout.write(`  ${color("⌥ executed:", GREEN)} id=${shortId(e2.result.id)} cost=$${e2.result.cost_usd.toFixed(4)}
`);
    }
    process.stdout.write(`
`);
  }
  async appendToFile(e2) {
    const line = JSON.stringify({
      ts: new Date().toISOString(),
      channel_label: e2.ctx.channel_label,
      triggering_event: e2.ctx.triggering_event,
      recent_event_count: e2.ctx.recent_events.length,
...
function previewEvent(evt) {
  ...
  return `${color(kind3, YELLOW)} from ${who}`;
}
function previewPayload(p2) {
  if (p2 == null)
    return "(empty)";
  if (typeof p2 === "string")
    return p2.length > 80 ? p2.slice(0, 77) + "..." : p2;
  if (typeof p2 === "object" && typeof p2.text === "string")
    return previewPayload(p2.text);
  const s2 = JSON.stringify(p2);
  return s2.length > 80 ? s2.slice(0, 77) + "..." : s2;
}
function formatDecision(d2) {
  switch (d2.kind) {
    case "noop":
      return `${dim("noop")} — ${d2.reason}`;
    case "react":
      return `${color("react", GREEN)} ${shortId(d2.signal_id)} ${JSON.stringify(d2.payload)} — ${d2.reason}`;
    case "push":
      return `${color("push", YELLOW)} ${shortId(d2.channel_id)} ${JSON.stringify(d2.payload)} — ${d2.reason}`;
  }
}
...
async function pushSignal(cfg, channelId, payload) {
  const resp = await authedFetch(cfg, `/channels/${channelId}/signals`, {
    method: "POST",
    body: JSON.stringify(payload)
  });
  if (!resp.ok)
    throw new Error(`pushSignal HTTP ${resp.status}: ${await resp.text()}`);
  const j2 = await resp.json();
  return { signal_id: j2.signal_id, channel_id: j2.channel_id, cost_usd: j2.cost_usd ?? 0 };
}
async function pushReaction(cfg, signalId, payload, isAuto = true) {
  const resp = await authedFetch(cfg, `/signals/${signalId}/reactions`, {
    method: "POST",
    body: JSON.stringify({ payload, is_auto: isAuto })
  });
  if (!resp.ok)
    throw new Error(`pushReaction HTTP ${resp.status}: ${await resp.text()}`);
  const j2 = await resp.json();
  return {
    reaction_id: j2.reaction_id,
    signal_id: j2.signal_id,
    channel_id: j2.channel_id,
    cost_usd: j2.cost_usd ?? 0
  };
}
async function recentSignals(cfg, channelId, limit2 = 20) {
  const resp = await authedFetch(cfg, `/channels/${channelId}/signals?limit=${limit2}`);
  if (!resp.ok)
...
class MinuteRateLimiter {
  ...
  tryConsume() {
    const now = Date.now();
    this.timestamps = this.timestamps.filter((t2) => now - t2 < 60000);
...
async function main() {
  ...
  try {
    ...
    if (meResp.ok) {
      const me2 = await meResp.json();
      myAddress = me2.address ?? null;
...
`);
  ...
  while (true) {
    ...
    for (const block of parts) {
      ...
      if (event === "ping" || event === "open" || event === "ejected")
        continue;
      if (!data)
        continue;
      let evt;
...
`);
  ...
  let history = [];
  try {
    const r2 = await recentSignals(susu, channelId, cfg.agent.history_per_channel);
    history = r2.signals ?? [];
  } catch {}
  const ctx = {
  ...
  try {
    ...
    process.stderr.write(`[daemon] LLM error: ${err?.message ?? err}
`);
    return;
  }
  let result;
  ...
  try {
    if (decision.kind === "react") {
      const r2 = await pushReaction(susu, decision.signal_id, decision.payload, true);
      result = { id: r2.reaction_id, cost_usd: r2.cost_usd };
      ...
      if (cfg.dry_run_pushes) {
        ...
        const r2 = await pushSignal(susu, decision.channel_id, decision.payload);
        result = { id: r2.signal_id, cost_usd: r2.cost_usd };
      }
    }
  } catch (err) {
